...
Status: early. The monorepo, CI gate, dev environment, task domain (state machine, activity timeline, tasks API), GitHub integration (webhook intake, repo sync, `/agent-trail run`), and runner loop (registration, expiring attempt leases, fake agent adapter) exist; the complete fake issue-to-PR flow is next in the milestone queue.
