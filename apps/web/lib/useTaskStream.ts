...
// server replays the whole timeline on first connect, EventSource resumes
// from Last-Event-ID after a drop, and a terminal task ends with a "done"
// event, after which the socket is closed for good.
...
export type StreamState =
  "connecting" | "live" | "reconnecting" | "done" | "error";
...
// after reports whether event a sorts strictly after cursor position b.
...
export function useTaskStream(taskId: string): TaskStream {
  ...
  useEffect(() => {
    ...
    es.onerror = () => {
      // CONNECTING means the browser is retrying with Last-Event-ID;
      // CLOSED means it gave up (the endpoint answered non-200).
      setState(es.readyState === EventSource.CLOSED ? "error" : "reconnecting");
    };
