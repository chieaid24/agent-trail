...
1. **Trigger** - a developer with `write` or `admin` on an enabled repository comments `/agent-trail run` on an issue. Node: Developer -> GitHub (App).
...
3. **Task creation** - the in-process processor parses the exact command, rejects bots and PR comments, checks repository enablement and commenter permission over the GitHub REST API, then in one transaction inserts `tasks`, `task_attempts` (attempt 1), and a `task.created` event, and transitions the task to `queued`. A unique index on active tasks per issue is the race guard. The API acknowledges with an issue comment. Node: API -> RDS PostgreSQL. Records: `tasks`, `task_attempts`, `activity_events`.
...
5. **Provisioning** - the worker mints a GitHub installation token, refreshes the bare mirror at `repos/<repository_id>/repo.git`, and creates a worktree at `workspaces/<task_attempt_id>` on branch `agent-trail/<label>` from the recorded base commit. Node: Workspace disk (emptyDir in Kubernetes). Record: `task_attempts.base_commit_sha`.
...
| GitHub check run | name `Agent Trail Task` | Conclusion `failure` on any failed trusted check, otherwise `neutral`; never `success` | Evidence report -> GitHub |
