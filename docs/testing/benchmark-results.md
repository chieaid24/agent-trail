...
- Date: 2026-08-19 (UTC), run log `bench-20260819T225026Z.log`
- Commit: `a5e4c81`
...
| Agent hang | Session emits one event, then nothing, until cancelled | A 1s task limit ended `timed_out` with `task_runtime_exceeded`; API cancellation stopped a second hung session and released its lease in 123ms; both workspaces were removed and no lease remained (supplemental run above) |
