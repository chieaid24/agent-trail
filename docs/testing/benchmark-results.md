...
- Date: 2026-08-19 (UTC), run log `bench-20260819T203640Z.log`
- Scope: `BENCH_RUN=TestInjectAgentHang` on the issue #36 worktree represented
  by this change
...
| Agent hang | Session emits one event, then nothing, until cancelled | A 1s task limit ended `timed_out` with `task_runtime_exceeded`; API cancellation stopped a second hung session and released its lease in 116ms; both workspaces were removed and no lease remained (supplemental run above) |
